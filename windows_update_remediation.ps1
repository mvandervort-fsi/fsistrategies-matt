...
#Run DISM
Repair-WindowsImage -RestoreHealth -NoRestart -Online -LogPath "C:\ProgramData\Microsoft\IntuneManagementExtension\Logs\#DISM.log" -Verbose
...
if  ($OSversion -match "10.0.1")
    ...
    if  ($OSversion -lt "10.0.19045")
        ...
        if  ($CheckWhyAmIBlocked.Name -eq "FU.WhyAmIBlocked")
            ...
            Install-Module FU.WhyAmIBlocked -Force -Verbose
            }
        Import-Module FU.WhyAmIBlocked -Verbose
        Get-FUBlocks -Verbose
        }
...
try {
    ...
    Reset-WUComponents -Verbose -ErrorAction Stop
}
catch {
    Write-Output "An error occurred while resetting Windows Update Components: $_"
}
...
try {
    ...
    Write-Output "An error occurred while checking for Windows updates: $_"
}
